...
- [System Architecture Curriculum](#system-architecture-curriculum)
  ...
  - [Phase 1: Foundation](#phase-1-foundation)
    - [1.1 Technical Core](#11-technical-core)
      - [1.1.1 Proficiency in 1-2 OOP/System](#111-proficiency-in-1-2-oopsystem)
  ...
  - [Phase 3: Structure Architect](#phase-3-structure-architect)
  ...
  - [Capstone Project](#capstone-project)
    ...
    - [Example Project Across Domains](#example-project-across-domains)
 ...
 * **Code Concepts**: Analysis of time/space complexity; masteryof trees, graphs, hash tables, and their real-world application in system design (e.g., indexes, routing).
 * **Practical Output**: Solve 75+ LeetCode problems (focus: Medium/Hard) and implement a feature that requires a non-trivial data structure (e.g., LRU, cache, trie).

#### 1.1.3 Development Tool Proficiency
 * **Core Concepts**: Advanced Git (rebasing, bisecting); Linux command0line mastery; scripting for automation (Bash/Python).
 ...
 * **Core Concepts**: Process vs. threads, scheduling, virtual memory, I/O, and interrupts.
 * **Practical Output**: Write a multi-threaded program that demonstrates a race condition and fix it with synchronisation primitives.
 ...
 * **Practical Output**: Use Wireshark to capture and analyse a TCP handshake and HTTP request. Diagram the complete flow of a user typing a URL to a page loading.

#### 1.2.3 Databases Fundamentals
 * **Core Concepts**: Relational design (normalisation, ACID), SQL proficiency (joins, subqueries, transactions), NoSQL paradigms (document, key-value, wide-column).
 * **Practical Output**: Design a normalised schema for a given domain. Benchmark and compare simple operations in SQL vs NoSQL databases.
 ...
 * **Core Concepts**: SOLID, DRY, YAGNI, composition over inheritance; Gang of Four patterns (Factory, Singleton, Observer, Strategy).
 ...
 * **Core Concepts**: Monoliths, Microservices, Event-Driven Architecture, Server-Oriented Architecture (SOA), Serverless. Trade-offs in coupling, cohesion, and complexity.
 ...
 * **Core Concepts**: RESTful principles, gRPC/protobuf, GraphQL schemas, messaging (Kafka, RabbitMQ), idempotency, versioning strategies.
 * **Practical Output**: Design a REST API with an OpenAPI spec and a gPRC service of the same domain. Implement a simple event-driven workflow using a message queue.
 ...
 * **Core Concepts**: IaaS vs PaaS vs SaaS, cloud economics, core services (compute, storage, networking, AIM) on AWS, Azure, or GCP.
 ...
 * **Practical Output**: "Dockerize" a multi-service application. Deploy it to a local Kubernetes cluster (minikube/kind) and then to a managed cloud service (EKS/GKE/AKS).
 ...
 * **Core Concepts**: The C4 Model for visualisation, UML sequence and component diagrams, writing effective Architecture Decision Records (ADRs) and Request for Comments (RFCs).
 * **Practical Output**: Create a C4 model (Context, Containers, Components, Code) for a system like a video streaming service. Write an ADR for a major technology choice within it.

#### 2.3.2 Non-Functional Requirements
 * **Core Concepts**: Scalability (horizontal/vertical), reliability (MTTF, MTTR, SLAs/SLOs/SLIs), performance, and security considerations.
 * **Practical Output**: Define SLOs for a service. Design a scalable architecture for a known problem (e.g. "Design a URL shortener") that addresses specific NFRs. Implement basic observability (logging, metrics) for your deployed app.

---

## Phase 3: Structure Architect
 ...
 * **Practical Output**: Develop a simplified future-state architecture roadmap for a small business model, linking technology initiatives to business capabilities.

#### 3.1.2 Risk, Security & Compliance
 * **Core Concepts**: Security-by-design, threat modelling (STRIDE), risk assessment frameworks, data privacy regulations (GDPR, CCPA), and industry standards (ISO 27001, SOC 2).
 * **Practical Output**: Conduct a threat modelling session for one of your previous designs. Draft a compliance checklist for a hypothetical application handling user data.

#### 3.1.3 Cost Optimization & Budgeting
 * **Core Concepts**: Cloud most models (CAPEX vs OPEX), TCO analysis, right-sizing, reserved vs spot instances, tagging strategies, and FinOps principles.
 * **Practical Output**: Analyse the cloud bill of a sample architecture. Propose and justify specific cost-saving changes, estimating the potential savings.
 ...
 * **Core Concepts**: Formalising the ADR process, decision log maintenance, communicating and socialising decisions, and reviewing past decisions.
 * **Practical Output**: Establish a lightweight ADR process for your personal or team projects. Maintain a decision for the major choices in your capstone project.
...
### Objective
Synthesise the learning from all three phases of the curriculum. **You will architect a system for a domain of your choice**, demonstrating mastery of technical design, strategic planning, and communication skills.
 ...
 * **Deliverable**: A one-page project charter.
   ...
   * Identify the primary **stakeholders** (e.g., end-users, operations, business owners).

#### Step 2: Comprehensive Architecture Design
 * **Task**: Create the complete architectural specification of your chosen system.
 * **Deliverables**:
   *  **C4 Model Diagrams**: A full set (Context, Containers, Components) visually defining the system's structure and interactions.
   *  **Architecture Decision Record (ADR) Log**: At least 5 ADRs documenting critical technology choices (e.g., database selection, communication protocol, cloud provider).
   *  **API Contract Specifications**: Detailed OpenAPI/Swagger specs for major services or a gPRC protocol definition.
   *  **Non-Functional Requirements (NFR) Plan**: Explicit SLOs/SLIs and a description of how scalability, reliability, and security will be achieved.

#### Step 3: Strategic & Business Integration
 * **Task**: Develop the business and operational context for your technical design.
 * **Deliverables**:
   * **Phased Implementation Roadmap**: A 3-phase plan (e.g., MVP, Scale, Optimise) with milestones and deliverables.
   * **Cost-Benefit Analysis & Budget Forecast**: A high-level TCO for the first year, identifying major cost drivers and potential optimisations.
...
### Example Project Across Domains
 * **Healthcare**: Design a **HIPAA-compliant patient data lake** for clinical research, focusing on data anonymisation, secure access, and batch/stream processing.
 * **IoT/Manufacturing**: Design a **predictive maintenance platform** for industrial equipment, focusing on real-time telemetry ingestion, time-series analysis, and alerting mechanisms.
 ...
 * **Present and Defend**: Present your architecture to a **review board** (peers, mentor, online community) in a 20-minute session, followed by a 10-minute Q&A. You must justify your decisions and explain trade-offs.
 * **Success Criteria**:
